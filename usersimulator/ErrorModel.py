...
class DomainsErrorSimulator(object):
    ...
    def _set_nbest_generator(self):
        if self.nBestGeneratorName == 'UniformNBestGenerator':
            ...
            #Fotis
            self.nBestGenerator_clean = NBestGenerator.EMSampledNBestGenerator(self.confusionModel, 0, self.nBestSize)
            ...
            #Fotis
            self.nBestGenerator_clean = NBestGenerator.DSTC2NBestGenerator(self.confusionModel, 0, self.nBestSize, self.paramset)
        else:
    ...
    def confuse_act(self, last_user_act):
        ...
        #Fotis
        n_best_clean = self.nBestGenerator_clean.getNBest(uact)

        n_best = self.nBestGenerator.getNBest(uact)
        #Fotis
        n_best_clean = self.confScorer.assignConfScores(n_best_clean)
        ...
        # Normalise confidence scores for clean| Fotis
        dSum = 0.0
        for hyp in n_best_clean:
            dSum += hyp.P_Au_O  # P_Au_O is the confidence score
        for hyp in n_best_clean:
            hyp.P_Au_O /= dSum
        
        return n_best, n_best_clean
