...
def get_configuration(domainString, policyType):
    ...
    config_filename = os.path.join('.',  'autoencoder', 'config', 'config_params.json')   
    ...
    for domain in cfg['domains']:
        prev_total_length = 0  
        total_length += int(cfg['state_dim'][domain])
        cfg['domain_indices'][domain] = np.array([prev_total_length, total_length], dtype=np.int32) 

    cfg['layers_shape'][0] = total_length #int(cfg['state_dim'][domainString])
