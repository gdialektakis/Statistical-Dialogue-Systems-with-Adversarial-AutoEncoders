...
"""
Variational Dense Denoising Autoencoder
"""
...
#        self.project_dim = cfg['project_dim']
        ...
        with tf.variable_scope("autoencoder_" + variable_scope_name, reuse=tf.AUTO_REUSE):
            ...
            if cfg['restore_model']:
                try:
                    ...
                    print("Variables restored for "+domainString+' VDAE ' + str(self.layers_shape))
                except:
                    print("Variables restore Failed for "+domainString+' VDAE!')
                    pass
    ...
    def create_variables(self):
        ...
        # mean & stdev matrixes
        get_weight_variable('W' + "mean", (self.layers_shape[-2], self.latent_size))

        get_weight_variable('W' + "stddev", (self.layers_shape[-2], self.latent_size))
    ...
    def encoder(self, X):
        ...
        W = get_weight_variable('W' + "mean")
        mean = tf.nn.dropout(tf.nn.tanh(tf.matmul(Y, W)), keep_prob=self.prob)
    ...
    def define_operations(self):
        ...
        mean, log_stddev = self.encoder(Y)
        self.latent_vector = self.sampler(mean, log_stddev)


        X_reconstructed = self.decoder(self.latent_vector)  # Network prediction

        # Create a scalar summary object for the encoder and decoder so it can be displayed
        self.tf_encode_summary = tf.summary.scalar('loss', self.latent_vector)
        self.tf_decode_summary = tf.summary.scalar('loss', X_reconstructed)

        # compute the avg MSE error, then scale it up, i.e. simply sum on all axes
        reconstruction_loss = tf.reduce_sum(tf.pow(self.X_data_clean_placeholder - X_reconstructed, 2), axis=-1)
        # compute KL loss
        ...
        self.train_loss = tf.reduce_mean(reconstruction_loss + kl_loss)
    ...
    def encode(self, X_data):
        X_encoded_np = self.sess.run(self.latent_vector, feed_dict={self.X_data_placeholder: X_data, self.prob: 1.0})
