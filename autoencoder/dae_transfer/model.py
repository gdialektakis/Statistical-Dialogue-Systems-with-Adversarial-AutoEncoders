...
def get_weight_variable(name, shape=None, initializer=tf.contrib.layers.xavier_initializer_conv2d(), trainable=True):
    if shape is None:
        return tf.get_variable(name, trainable=trainable)
    else:
        return tf.get_variable(name, shape=shape, dtype=_FLOATX, initializer=initializer, trainable=trainable)
...
#        self.project_dim = cfg['project_dim']
        ...
        with tf.variable_scope("autoencoder_" + variable_scope_name, reuse=tf.AUTO_REUSE):
            ...
            if cfg['restore_model']:
                try:
                    ...
                    print("Variables restored for "+domainString+' DAE Transfer' + str(self.layers_shape[1:]))
                except:
                    print("Variables restore Failed for "+domainString+' DAE Transfer!')
                    pass
    ...
    def create_variables(self):
        ...
        #else:
        get_weight_variable('W_' + self.domain, (self.state_dim[self.domain], self.layers_shape[1]))

        for i in range(1, self.n_layers):
            W_name = 'W' + str(i)
            self.core_variable_save_list[W_name] = get_weight_variable(W_name, (self.layers_shape[i], self.layers_shape[i + 1]), trainable=self.trainable)
    ...
    def define_operations(self):
        ...
        if self.trainable:
            #This saves only the hidden layers to perform transfer learning when training in multiple domains
            self.saver = tf.train.Saver(var_list=self.core_variable_save_list, max_to_keep=1)
        else:
            # this saves the whole NN in the case of a stable AE when we want to only adjust the input and output layer
    ...
    def train(self, input_x, clean_input_x):
        ...
        print('DAE Transfer ' + self.domainString + ' mean loss:', mean_loss)
    ...
    def save_variables(self):
        ...
        shape_str = ''
        for i in self.cfg['layers_shape'][1:]:
            shape_str += str(i) + '_'
        ...
        #saves the common hidden layers to a common file
        if self.trainable:
            model_path = os.path.join(self.cfg['base_dir'], self.cfg['saved_models_dir'],
                                  self.cfg['policyType'], str(self.cfg['model_id']),
                                  shape_str[:-1])
        else:
            model_path = os.path.join(self.cfg['base_dir'], self.cfg['saved_models_dir'], self.domain,
                                      self.cfg['policyType'], str(self.cfg['model_id']),
                                      shape_str[:-1])
    ...
    def restore_variables(self):
        ...
        saver = tf.train.Saver(self.core_variable_save_list)
