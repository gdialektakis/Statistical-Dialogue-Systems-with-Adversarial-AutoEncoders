...
class Autoencoder(object):
    ...
    def __init__(self, domainString, policyType):
        ...
        with tf.variable_scope("autoencoder_multi", reuse=tf.AUTO_REUSE):
            ...
            if cfg['restore_model']:
                try:
                    ...
                    print("Variables restored for "+domainString+' AE')
                except:
                    print("Variables restore Failed for "+domainString+' AE!')
                    pass

    def create_variables(self):
        if self.isMulti:
            get_weight_variable('W_project_input', (self.input_dim, self.project_dim))
            get_weight_variable('W0', (self.project_dim, self.layers_shape[1]))
        else:
            get_weight_variable('W_' + self.domainString, (self.state_dim[self.domainString], self.layers_shape[1]))

        for i in range(1, self.n_layers):
            get_weight_variable('W' + str(i), (self.layers_shape[i], self.layers_shape[i + 1]))

    def encoder(self, X):
        W = get_weight_variable('W0')
    ...
    def decoder(self, Y):
        ...
        if self.isMulti:
            W = get_weight_variable('W_project_input')
            X = tf.nn.relu(tf.matmul(X, tf.transpose(W)))
        else:
    ...
    def define_operations(self):
        ...
        self.learning_rate = get_learning_rate(self.cfg['learning_rate_method'], self.global_step,
                                               self.learning_rate_params)
    ...
    def saveToStateBuffer(self, state):
        self.state_buffer.append(state.reshape((1, -1)))
