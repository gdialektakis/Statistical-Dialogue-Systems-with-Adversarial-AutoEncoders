...
def get_configuration(domainString, policyType):
    ...
    config_filename = os.path.join('.',  'autoencoder', 'config', 'config_params_multi.json')
    ...
    elif cfg['multi_transfer']:
        cfg['input_dim'] = 300
        cfg['layers_shape'][0] = cfg['input_dim']
    else:
        # fotis
        input_dim = 0
        for domain in cfg['domains']:
            input_dim += cfg['state_dim'][domain]
        cfg['input_dim'] = input_dim

        cfg['layers_shape'][0] = int(cfg['project_dim'])
        cfg['layers_shape'] = np.concatenate((input_dim, cfg['layers_shape']), axis=None)
