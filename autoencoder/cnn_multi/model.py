...
class Autoencoder(object):
    ...
    def __init__(self, domainString, policyType):
        ...
        self.autoencoder_trainable_vars = []
    ...
    def create_variables(self):
        ...
        for i in range(self.n_conv_layers):
            ...
            get_weight_variable('W_deconv2d_' + str(i+1), (1, 3, self.n_channels_list[i], self.n_channels_list[i+1]))
        ...
        for i in range(self.n_dense_layers):
            get_weight_variable('W_dense_encoder_' + str(i+1), (self.dense_layers_shape[i], self.dense_layers_shape[i+1])) 
            get_bias_variable('b_dense_encoder_' + str(i+1), shape=(self.dense_layers_shape[i+1]))

            get_weight_variable('W_dense_decoder_' + str(i+1), (self.dense_layers_shape[i+1], self.dense_layers_shape[i]))
    ...
    def encoder(self, X):
        ...
        for i in range(self.n_conv_layers):
            ...
            """Here becomes from 257->256 the dimension. Is it because of the avg pooling?"""
    ...
    def decoder(self, Y):
        ...
        for i in range(self.n_conv_layers-1, -1, -1):
            ...
            X = tf.nn.conv2d_transpose(X, W, output_shape=output_shape, strides=[1, 1, self.stride, 1], padding='SAME') + b
