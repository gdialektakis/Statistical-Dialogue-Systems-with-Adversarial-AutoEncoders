...
def get_configuration(domainString, policyType):
    ...
    config_filename = os.path.join('.',  'autoencoder', 'config', 'config_params_cnn_spp.json')   
    ...
    cfg['dense_input_dim'] = sum(cfg['n_bins_list'])*cfg['n_channels_list'][-1]
    cfg['dense_layers_shape'][0] = cfg['dense_input_dim']
    cfg['input_dim'] = int(cfg['state_dim'][domainString])

    non_linearity = cfg['non_linearity'].lower()
    if non_linearity == 'relu':
        cfg['non_linearity_fn'] = tf.nn.relu
    elif non_linearity == 'tanh':
        cfg['non_linearity_fn'] = tf.nn.tanh
    elif non_linearity == 'sigmoid':
        cfg['non_linearity_fn'] = tf.nn.sigmoid
