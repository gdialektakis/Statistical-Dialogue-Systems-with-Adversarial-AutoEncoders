...
class Policy(object):
    ...
    def act_on(self, state, state_clean):
        '''
        ...
        #Fotis
        clean_beliefstate = state_clean.getDomainState(self.domainString)

        if self.lastSystemAction is None and self.startwithhello:
            _systemAct = 'hello()'
        #elif self.domainString == 'topicmanager':
        #    _systemAct = self.nextAction(beliefstate)
        else:
            _systemAct = self.nextAction(beliefstate)
        self.lastSystemAction = _systemAct
        self.prevbelief = beliefstate
        self.prevbelief_clean = clean_beliefstate
        
        systemAct = DiaAct.DiaAct(_systemAct)
        return systemAct
    
    def record(self, reward, domainInControl = None, weight = None, state=None, state_clean=None, action=None):
        '''
        ...
        if state_clean is None:
            state_clean = self.prevbelief_clean
        if action is None:
            action = self.actToBeRecorded
            
        cState, cAction = self.convertStateAction(state, action)
        cState_clean, _ = self.convertStateAction(state_clean, action)
        
        if weight == None:
            self.episodes[domainInControl].record(state=cState, state_clean=cState_clean, action=cAction, reward=reward)
        else:
            self.episodes[domainInControl].record(state=cState, state_clean=cState_clean, action=cAction, reward=reward, ma_weight = weight)
...
class Episode(object):
    ...
    def record(self, state, action, reward, ma_weight=None, state_clean=None):
        '''
        ...
        self.strace_clean.append(state_clean)
