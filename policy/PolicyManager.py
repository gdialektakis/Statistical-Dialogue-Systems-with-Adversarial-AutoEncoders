...
#         self.lastSystemAction = None
        ...
        # Modifications for autoencoder | Fotis
        self.autoencoder = None
        if Settings.config.has_option('exec_config', 'autoencoder') and Settings.config.getboolean(
                'exec_config', 'autoencoder'):
            # check if whether to use transfer AE or not
            if Settings.config.has_option('exec_config', 'transfer') and Settings.config.getboolean('exec_config', 'transfer'):
                if Settings.config.has_option('exec_config', 'transfer_autoencoder_type'):
                    autoencoder_type = Settings.config.get('exec_config', 'transfer_autoencoder_type')
                    self.autoencoder = self._init_transfer_autoencoder(autoencoder_type)
        # End of modifications

    def _init_transfer_autoencoder(self, autoencoder_type=None):
        ...
        if Settings.config.has_option('GENERAL', 'singledomain'):
            if not Settings.config.getboolean('GENERAL', 'singledomain'):
                autoencoder = Autoencoder(domainString="multi", policyType="gp")
                Settings.config.set('exec_config', 'reuse', 'True')
            else:
                domain = Settings.config.get('GENERAL', 'domains')
                autoencoder = Autoencoder(domainString=domain, policyType="gp",
                                             variable_scope_name=domain)

        # fotis
        self.pretrainAE = False
        if Settings.config.has_option('exec_config', 'pretrainAE'):
            self.pretrainAE = Settings.config.getboolean('exec_config', 'pretrainAE')
        if self.pretrainAE:
            autoencoder.loadEpisodes()
    ...
    def act_on(self, dstring, state, state_clean):
        '''
        ...
        if self.committees[dstring] is not None:
            ...
            #gdiale modification for Feudal

            self.is_feudal = False
            if Settings.config.has_option('policy', 'feudal'):
                self.is_feudal = Settings.config.getboolean('policy', 'feudal')

            if self.is_feudal:
                systemAct = self.domainPolicies[dstring].act_on(state=state)
            else:
                systemAct = self.domainPolicies[dstring].act_on(state=state, state_clean=state_clean)
    ...
    def finalizeRecord(self, domainRewards,currentDomain):
        '''
        ...
        for dstring in self.domainPolicies:
            if self.domainPolicies[dstring] is not None:
                ...
                if domains_reward is not None:
                    ...
                    #Here I put code
                    elif self.domainPolicies[dstring] is not None and currentDomain == dstring:
...
#         self.prevbelief = None
    ...
    def _load_domains_policy(self, domainString=None):
        ...
        if domainString in self.SPECIAL_DOMAINS:
            ...
            elif policy_type == 'Parrot':
                from policy import ParrotPolicy
                self.domainPolicies[domainString] = ParrotPolicy.Parrot(domainString)
            elif policy_type == 'lspi':
                from policy import LSPIPolicy
                self.domainPolicies[domainString] = LSPIPolicy.LSPIPolicy(domainString, learning)
            else:
                try:
