...
def initialise(configId,config_file, seed, mode, trainerrorrate=None, trainsourceiteration=None,
    ...
    if mode == "train":
        if gnumtrainbatches>1:
            ...
            logfile = "%s-%s%02d.%02d.%d-%d.train.log" % (configId, seed_string,gtrainerrorrate, gtesterrorrate,gtrainsourceiteration+1, enditeration)
        else:
            logfile = "%s-%s%02d.%02d.%d.train.log" % (configId, seed_string, gtrainerrorrate, gtesterrorrate, gtrainsourceiteration + 1)
    elif mode == "eval":
        if testenderrorrate:
...
def train_command(configfile, seed=None, trainerrorrate=None,testerrorrate=None, trainsourceiteration=None,
    ...
    try:
        if seed and seed.startswith('('):
            ...
            for seed in seeds:
                ...
                train_command(configfile, seed=seed, trainerrorrate=trainerrorrate, testerrorrate=testerrorrate,
            ...
            for i in range(gtrainsourceiteration, gtrainsourceiteration+gnumtrainbatches):
                ...
                if gtesteverybatch and gnumbatchtestdialogs > 0 and i+1 < gtrainsourceiteration+gnumtrainbatches:
                    if isSingleDomain:
                        evalPolicy(domain, configId, gtesterrorrate, gnumbatchtestdialogs, i + 1, seed=seed)
                    else:
                        evalPolicy(domains, configId, gtesterrorrate, gnumbatchtestdialogs, i + 1, seed=seed)
    ...
    try:
        ...
        if ncurves>0:
            ...
            if len(domains) > 1:
                ...
                plotTrain(datasetname+'-mean', av_rtab, av_stab, av_ttab, block=block,saveplot=saveplot)
        else:
