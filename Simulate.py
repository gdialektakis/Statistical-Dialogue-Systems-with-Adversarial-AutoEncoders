...
class SimulationSystem(object):
    ...
    def run(self, session_id, agent_id='Smith', sim_level='dial_act'):
        ...
        while not endingDialogue:
            i+=1
            #print("Inside Dialogue Loop. Turn %d" % i)
            # USER ACT:  
            #-------------------------------------------------------------------------------------------------------------
            sys_act = self.agent_factory.agents[agent_id].retrieve_last_sys_act()
            
            user_act, user_actsDomain, hyps, clean_hyp = self.simulator.act_on(sys_act)
            #print("User Act Domain: "+ user_actsDomain)
            if sim_level == 'text':
            ...
            # SYSTEM ACT:
            #-------------------------------------------------------------------------------------------------------------
            sys_act = self.agent_factory.agents[agent_id].continue_call(asr_info=hyps, asr_info_clean=clean_hyp,
        ...
        for domain in self.simulator.simUserManagers:
            if self.simulator.simUserManagers[domain]:
                ...
                logger.dial('User goal at the end of the dialogue: {}'
                            .format(str(goal.request_type) + str(goal.constraints) + str([req for req in goal.requests])))
        self.agent_factory.agents[agent_id].end_call(domainSimulatedUsers=self.simulator.simUserManagers, domainString=user_actsDomain)
